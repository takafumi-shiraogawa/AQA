...
def proc_hessian_(mf, mf_hess, mo_energy=None, mo_coeff=None, mo_occ=None, h1ao_grad=None):
    """
    ...
    moao1_grad, mo_e1_grad = mf_hess.get_response_matrix(mo_energy, mo_coeff, mo_occ, h1ao_grad)
...
def make_U_R(mf):
    # TODO: More elegant coding is possible by using base with PySCF 2.
    # mf.mf_hess = mod_hessian_rhf.Hessian(mf.base)
    mf_hess = mod_hessian_rhf.Hessian(mf)
    # mf_hess, h1ao_grad, mo1_grad, mo_e1_grad = proc_hessian_(mf.mf_hess)
    mf_hess, h1ao_grad, mo1_grad, mo_e1_grad = proc_hessian_(mf, mf_hess)